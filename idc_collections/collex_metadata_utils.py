...
def get_bq_metadata_api(filters, fields, data_versions):
    results = {}
    filter_attr_by_bq = {}
    field_attr_by_bq = {}

    query_base = """
        SELECT DISTINCT {field_clause}
        FROM {table_clause}
        {join_clause}
        {where_clause}
    """
...
#    field_attrs = Attribute.objects.filter(active=True, name__in=fields)
    ...
    field_clause = ",".join(list(field_clauses.values()))
...
#    param_sfx = 0
    for bqtable in filter_attr_by_bq:
        ...
        filter_clauses[bqtable] = BigQuerySupport.build_bq_filter_and_params(
            filter_set, field_prefix=table_info[bqtable]['alias'],
            case_insens=True
            )

    filter_clause = " AND ".join([filter_clauses[clause]['filter_string'] for clause in filter_clauses ])
    params = [filter_clauses[clause]['parameters'] for clause in filter_clauses ]

    tables = list(table_info.keys())
    prev_table = tables[0]
    table_clause = "`{}` {}".format(prev_table,table_info[prev_table]['alias'])
    join_clause = ""
    for table in tables[1:]:
        join_clause += "JOIN `{}` {}\n".format(table,table_info[table]['alias'])
        join_clause += "\t\tON {}.{} = {}.{}\n\t\t".\
            format(table_info[prev_table]['alias'],table_info[prev_table]['id_col'],
                   table_info[table]['alias'],table_info[table]['id_col'])
        prev_table = table
    # Drop the final /n/t/t
    join_clause = join_clause[:-3]

    full_query_str = query_base.format(
        field_clause = field_clause,
        table_clause = table_clause,
        join_clause = join_clause,
        where_clause = "WHERE {}".format(filter_clause if filters else "")
    )
...
def get_bq_string_api(filters, fields, data_versions):
    filter_attr_by_bq = {}
    field_attr_by_bq = {}
    ...
    # query_base = """
    #     SELECT {field_clause}
    #     FROM `isb-cgc.TCGA_bioclin_v0.Biospecimen` biospecimen
    #     JOIN `isb-cgc.TCGA_bioclin_v0.Clinical` clinical
    #     ON biospecimen.case_barcode = clinical.case_barcode
    #     JOIN `idc-dev-etl.tcia.dicom_metadata` dicom_metadata
    ...
    filter_clause = " AND ".join(list(filter_clauses.values()))
